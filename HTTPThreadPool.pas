 ...
 * @version $Id: HTTPThreadPool.pas 48 2011-01-13 15:14:38Z WladiD $
 *}
unit HTTPThreadPool;
 ...
 *
 * @param PostData Optional. The passed stream is managed by the task and will be destroyed
 *        automatically.
 *}
...
begin
	...
	begin
		...
		ConnectTimeout:=NetAccessConfig.ConnectTimeout;

		if NetAccessConfig.UseProxy then
		begin
			with ProxyParams do
			begin
				ProxyServer:=NetAccessConfig.ProxyServer;
				ProxyPort:=NetAccessConfig.ProxyPort;
				if NetAccessConfig.ProxyRequireAuth then
				begin
					BasicAuthentication:=TRUE;
					ProxyUsername:=NetAccessConfig.ProxyUsername;
					ProxyPassword:=NetAccessConfig.ProxyPassword;
				end;
			end;
		end;
		Request.UserAgent:=NetAccessConfig.UserAgent;
	end;
end;
